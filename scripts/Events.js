!function(){
    ...
    Events = function Events(el){
        ...
        this.addEvent('destroy:after',function (e){
    ...
    Events.Pseudoes = {
        ...
        after : {
            addEvent : function(type, fn){
                if (!this.$after[type]) this.$after[type] = [];

                this.$after[type].push(fn);    
            }    
        }
    };
    ...
    fireEvent = compat ?
        function(type, args){
            ...
            if (this.$after[data.name]){
                for (i=0; fn = this.$after[data.name][i]; i++){
                    setTimeout(after(fn,ev,data.name),0);    
                }
            }
